...
mllm = OpenAIModel(base_url="https://api.openai.com/v1/chat/completions",
                       api_key="",  # Set the value to sk-xsxx if you host the openai interface for open llm model,
...
while round_count < 20:
    ...
    if status:
        ...
        if decision == "INEFFECTIVE":
            ...
            last_act = "None"
        elif "SUCCESS" in decision or "BACK" in decision or "CONTINUE" in decision:
            if "BACK" in decision or "CONTINUE" in decision:
                useless_list.add(resource_id)
                last_act = "None"
                if decision == "BACK":
                    ret = controller.back()
